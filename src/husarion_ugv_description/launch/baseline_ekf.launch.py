...
def generate_launch_description():
    ...
    # =========================================================================
    # Stage 1: Odometry sources + covariance relays
    # =========================================================================
    ...
    imu_relay = Node(
        package="husarion_ugv_description",
        executable="imu_covariance_relay.py",
        name="imu_covariance_relay",
        output="screen",
        parameters=[{"use_sim_time": True}],
    )

    gate_relays = make_topic_gate(
        "gate_relays",
        ["/imu/data/covariance_fixed", "/gps/fix/covariance_fixed"],
        timeout_s=60,
    )
    ...
    return LaunchDescription([
        ...
        make_stage(gate_sensors, [odom_sources, gps_relay, imu_relay, gate_relays],
                   "sensors→relays"),

        # Stage 2 — fires when gate_relays exits 0
        make_stage(gate_relays, [ekf_local, ekf_map, gate_ekf],
                   "relays→EKF"),
